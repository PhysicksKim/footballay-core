...
@Service
public class MatchDataAlertNotifier implements AlertNotifier {

    private static final Logger log = LoggerFactory.getLogger(MatchDataAlertNotifier.class);
    ...
    @Value("${discord.webhook.match:}")
    private String DISCORD_MATCH_WEBHOOK_URL;

    public MatchDataAlertNotifier(DiscordWebhookSender discordWebhookSender,
    ...
    public boolean isSupport(AlertCategory category) {
        return category.equals(AlertCategory.MATCHDATA);
    }
    ...
    private void notifySuccess(String fixtureId, String message) {
        ...
        discordWebhookSender.sendWebhook(DISCORD_MATCH_WEBHOOK_URL, request);
    }

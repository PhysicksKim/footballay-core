...
import com.footballay.core.domain.football.external.fetch.response.FixtureSingleResponse._FixtureSingle;
import com.footballay.core.domain.football.external.fetch.response.FixtureSingleResponse._Statistics;
...
import com.footballay.core.domain.football.repository.live.ExpectedGoalsRepository;
...
import static com.footballay.core.domain.football.external.fetch.response.FixtureSingleResponse._Away;
import static com.footballay.core.domain.football.external.fetch.response.FixtureSingleResponse._Home;
...
public class TeamStatisticsService {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TeamStatisticsService.class);
    ...
    private Map<Long, _Statistics> mapStatisticsByTeamId(List<_Statistics> statisticsList) {
        return statisticsList.stream().collect(Collectors.toMap(stat -> stat.getTeam().getId(), stat -> stat));
    }
     ...
     * @param response
     * @see TeamStatistics
     * @see com.footballay.core.domain.football.persistence.live.ExpectedGoals
     */
    public void saveTeamStatistics(FixtureSingleResponse response) {
        ...
        if (homeStatisticsResponse == null || awayStatisticsResponse == null) {
            log.error("Home or away statistics not matched with home or away team. Home ID: {}, Away ID: {}", home.getId(), away.getId());
            throw new IllegalArgumentException("Home or away statistics not matched with home or away team");
        }
        // Find exist entities
        Fixture fixture = fixtureRepository.findById(fixtureSingle.getFixture().getId()).orElseThrow(() -> new IllegalArgumentException("Fixture not found"));
        Team homeTeam = teamRepository.findById(home.getId()).orElseThrow(() -> new IllegalArgumentException("Home team not found"));
        Team awayTeam = teamRepository.findById(away.getId()).orElseThrow(() -> new IllegalArgumentException("Away team not found"));
        ...
        TeamStatistics homeStatistics;
        TeamStatistics awayStatistics;
    ...
    private void addOrUpdateXgToList(String xgValue, Integer elapsed, TeamStatistics teamStatistics) {
        ...
        ExpectedGoals newXg = ExpectedGoals.builder().elapsed(elapsed).xg(xgValue).teamStatistics(teamStatistics).build();
        xgList.add(newXg);
    ...
    private void addXgToList(List<ExpectedGoals> xgList, _Statistics homeStatisticsResponse, Integer elapsed, TeamStatistics teamStatistics) {
        ...
        ExpectedGoals xg = ExpectedGoals.builder().elapsed(elapsed).xg(xgValue).teamStatistics(teamStatistics).build();
        xgList.add(xg);
    }

    private TeamStatistics createTeamStatistics(_Statistics statistics, Fixture fixture, Team team) {
        TeamStatistics teamStatistics = TeamStatistics.builder().fixture(fixture).team(team).build();
    ...
    public TeamStatisticsService(final TeamStatisticsRepository teamStatisticsRepository, final TeamRepository teamRepository, final FixtureRepository fixtureRepository, final ExpectedGoalsRepository expectedGoalsRepository) {

...
public class FootballRoot {
    ...
    public Optional<LiveStatusDto> getFixtureLiveStatus(long fixtureId) {
        try {
            ...
            log.warn("error while getting FixtureLiveStatus by Id :: {}", e.getMessage());
            return Optional.empty();
        }
    }
    ...
    public List<FixtureEventWithPlayerDto> getFixtureEvents(long fixtureId) {
        try {
            ...
            log.error("error while getting FixtureEvents by Id :: {}", e.getMessage());
            return List.of();
        }
    }
    ...
    public List<TeamDto> getTeamsOfPlayer(long playerId) {
        try {
            ...
            log.error("error while getting PlayerTeamRelations by Id :: {}", e.getMessage());
            return List.of();
        }
    }

    /**
     * fixture 와 함께 통계 정보를 가져옵니다. <br>
     *
     * @param fixtureId 조회할 fixtureId
    ...
    public @Nullable MatchStatisticsDto getMatchStatistics(long fixtureId) {
        ...
        try {
            ...
            log.error("error while getting MatchStatistics by Id", e);
            return null;
        }
    }

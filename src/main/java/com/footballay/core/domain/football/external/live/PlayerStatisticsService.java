...
import static com.footballay.core.domain.football.external.fetch.response.FixtureSingleResponse._FixturePlayers._PlayerStatistics;

@Transactional
@Service
public class PlayerStatisticsService {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PlayerStatisticsService.class);
     ...
     */
    @NotNull
    private static Map<String, _PlayerStatistics> collectUnregisteredPlayerStats(List<_PlayerStatistics> playerStatisticsList) {
        return playerStatisticsList.stream().filter(ps -> (ps.getPlayer() == null || isStatsUnregisteredPlayer(ps.getPlayer().getId())) && StringUtils.hasText(ps.getPlayer().getName())).collect(Collectors.toMap(ps -> ps.getPlayer().getName(), Function.identity()));
    }

    @NotNull
    private static Map<Long, _PlayerStatistics> collectRegisteredPlayerStats(List<_PlayerStatistics> playerStatisticsList) {
        return playerStatisticsList.stream().filter(ps -> ps.getPlayer() != null && !isStatsUnregisteredPlayer(ps.getPlayer().getId())).collect(Collectors.toMap(ps -> ps.getPlayer().getId(), Function.identity()));
    }

    @NotNull
    private static Map<String, MatchPlayer> collectUnregisteredPlayers(List<MatchPlayer> matchPlayers) {
    ...
    @NotNull
    private static Map<Long, MatchPlayer> collectRegisteredPlayers(List<MatchPlayer> matchPlayers) {
        return matchPlayers.stream().filter(mp -> mp.getPlayer() != null).collect(Collectors.toMap(mp -> mp.getPlayer().getId(), Function.identity()));
    }

    private void createPlayerStatisticsEntityAndPutToMap(Map<?, MatchPlayer> givenMatchPlayers, Map<?, _PlayerStatistics> givenPlayerStatsData, List<PlayerStatistics> collectingPlayerStatsMap, List<MatchPlayer> collectingMatchPlayerMap) {
        for (Map.Entry<?, MatchPlayer> entry : givenMatchPlayers.entrySet()) {
            ...
            saveOrUpdatePlayerStatistics(collectingPlayerStatsMap, collectingMatchPlayerMap, matchPlayer, playerStatsData);
        }
    }

    private void saveOrUpdatePlayerStatistics(List<PlayerStatistics> collectingPlayerStatsMap, List<MatchPlayer> collectingMatchPlayerMap, MatchPlayer matchPlayer, _PlayerStatistics playerStatsData) {
    ...
    public PlayerStatisticsService(final TeamRepository teamRepository, final FixtureRepository fixtureRepository, final PlayerStatisticsRepository playerStatisticsRepository, final MatchLineupRepository matchLineupRepository, final MatchPlayerRepository matchPlayerRepository) {

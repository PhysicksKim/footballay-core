...
import com.footballay.core.web.common.dto.CachedApiResponse;
...
import com.footballay.core.web.football.response.TeamsOfLeagueResponse;
...
import com.footballay.core.web.football.service.FootballStreamWebService;
...
public class FootballStreamDataController {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(FootballStreamDataController.class);
    ...
    @GetMapping("/fixtures")
    public ResponseEntity<ApiResponse<FixtureOfLeagueResponse>> fixturesOnNearestDateFromNow(@ModelAttribute FixtureOfLeagueRequest request, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
    ...
    public ResponseEntity<ApiResponse<FixtureOfLeagueResponse>> fixturesOnDate(@ModelAttribute FixtureOfLeagueRequest request, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
    ...
    public ResponseEntity<ApiResponse<FixtureOfLeagueResponse>> availableFixturesOnNearestDateFromNow(@ModelAttribute FixtureOfLeagueRequest request, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
    ...
    public ResponseEntity<ApiResponse<FixtureOfLeagueResponse>> availableFixturesOnDate(@ModelAttribute FixtureOfLeagueRequest request, @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
    ...
    public ResponseEntity<ApiResponse<FixtureLineupResponse>> fixturesLineup(@RequestParam long fixtureId, @RequestParam(required = false) String preferenceKey) {
    ...
    public ResponseEntity<?> fixturesStatistics(@RequestParam long fixtureId, @RequestParam(required = false) String preferenceKey) {
        ...
        try {
            ...
            if (cachedResponseIfExist.isPresent()) {
                MetaData successMetaData = apiCommonResponseService.createSuccessMetaData(requestUrl, Map.of("fixtureId", String.valueOf(fixtureId)));
                ...
                return ResponseEntity.ok().header("X-Cache", "HIT").body(cacheResp);
            }
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse<Void>> handleMissingServletRequestParameterException(MissingServletRequestParameterException ex, HttpServletRequest request) {
        ...
        Map<String, String> requestParams = request.getParameterMap().entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> String.join(",", entry.getValue())));
        log.info("Request Parameter Exception occurred");
        log.info("Request URL : {} , Params : {}", requestUrl, requestParams);
        log.info("Exception: ", ex);
        ApiResponse<Void> failureResponse = apiCommonResponseService.createFailureResponse("Required request parameter is missing: " + ex.getParameterName(), requestUrl, requestParams);
    ...
    public ResponseEntity<ApiResponse<Void>> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex, HttpServletRequest request) {
        ...
        log.info("Request URL : {} , Params : {}", requestUrl, requestParams);
        log.info("Exception: ", ex);
        ApiResponse<Void> failureResponse = apiCommonResponseService.createFailureResponse("Invalid request parameter: " + ex.getName() + " with value " + ex.getValue(), requestUrl, requestParams);
        return ResponseEntity.badRequest().body(failureResponse);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleGenericException(Exception ex, HttpServletRequest request) {
        ...
        ApiResponse<Void> failureResponse = apiCommonResponseService.createFailureResponse("An unexpected error occurred", requestUrl, requestParams);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(failureResponse);
    }

    public FootballStreamDataController(final FootballStreamWebService footballStreamWebService, final ApiCommonResponseService apiCommonResponseService, final CachedApiResponseService cachedApiResponseService) {
        this.footballStreamWebService = footballStreamWebService;

...
class PlayerApiSportsWithCoreSyncer(
    ...
    @Transactional
    override fun syncPlayersOfTeam(teamApiId: Long, dtos: List<PlayerApiSportsCreateDto>): List<PlayerApiSports> {
        ...
        val processedPlayerApiSportsList = processPlayerApiSportsEntitiesBatch(dtos, existingPlayerApiSportsMap, teamCore)
    ...
    private fun createPlayerCoresBatch(cases: PlayerProcessingCases): Map<Long, PlayerCore> {
        ...
        playerCoreCreatePairs.addAll(cases.apiOnlyPlayers.map { it.first.apiId to it.second }.filter { it.first != null }.map { it.first!! to it.second })
     ...
     */
    private fun createTeamPlayerRelationshipsBatch(teamCore: TeamCore, playerCoreMap: Map<Long, PlayerCore>) {
    ...
    private fun prepareAllPlayerApiSports(
        ...
        cases.bothNewDtos.forEach { dto ->
            ...
            if (playerCore != null && dto.apiId != null) {
                val newPlayerApiSports = PlayerApiSports(
     ...
     */
    private fun saveAllPlayerApiSports(allPlayerApiSports: Map<Long, PlayerApiSports>): List<PlayerApiSports> {
        return playerApiRepository.saveAll(allPlayerApiSports.values)
    }
    ...
    private fun createPlayerCore(dto: PlayerApiSportsCreateDto): PlayerCore {
        return playerCoreSyncService.savePlayerCore(dto)
    }
} 
...
class PlayerApiSportsSyncer (
    ...
    fun syncPlayersOfTeam(teamApiId: Long, dtos: List<PlayerApiSportsCreateDto>) {
        ...
        // 모든 DTO의 apiId 목록
        val playerApiIds = dtos.map { it.apiId }
        ...
        updateTeamPlayerRelationships(team, processedPlayerApiSportsList, playerApiIds)
    ...
    private fun processPlayerApiSportsEntities(
        ...
        return playerApiSportsRepository.saveAll(result)
    }
    ...
    private fun createNewPlayerApiSports(
        ...
        createTeamPlayerRelationship(team, savedPlayerCore)
    ...
    private fun updateTeamPlayerRelationships(
        ...
        // 4. 제거되어야 할 연관관계 삭제 (이적 등의 이유)
        // 주석에 따르면 "dto 에 apiId 존재하지 않는 선수 - Team 연관관계 있음 : 아무 작업도 하지 않습니다."
        // 즉, 선수가 DTO에 없어도 기존 연관관계는 유지한다는 정책이므로 삭제 로직은 주석 처리
        
        // Note: PlayerCore가 더 이상 apiId를 갖지 않으므로 비교할 수 없습니다.
        // 선수 제거는 별도의 API로 처리하거나, PlayerApiSports를 통해 비교해야 합니다.
    }
}
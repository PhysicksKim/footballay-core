...
class EventSyncer : MatchEventDtoExtractor {
    ...
    private fun extractLineupInfo(dto: FullMatchSyncDto): LineupInfo {
        ...
        val homeStartPlayers = homeLineup.startXI.filter{ it.player.name != null }
                .associate { MatchPlayerKeyGenerator.generateMatchPlayerKey(it.player.id, it.player.name!!) to it }.toMutableMap()
    ...
    private fun toMatchEventDto(event: FullMatchSyncDto.EventDto, sequence: Int): MatchEventDto {
        return MatchEventDto(
            ...
            playerMpKey = if (event.player?.name != null) MatchPlayerKeyGenerator.generateMatchPlayerKey(
                event.player.id,
                event.player.name
            ) else null,
            assistMpKey = if (event.assist?.name != null) MatchPlayerKeyGenerator.generateMatchPlayerKey(
                event.assist.id,
                event.assist.name
            ) else null
        )

package com.footballay.core.backbone.core.relation
...
class TeamPlayerCoreSyncServiceImpl(
    ...
    @Transactional
    override fun createTeamPlayerRelationship(
        teamCore: TeamCore,
        playerCore: PlayerCore,
    ) {
        val teamId = teamCore.id ?: throw IllegalStateException("TeamCore must be saved before creating relationship")
        val playerId =
            playerCore.id ?: throw IllegalStateException("PlayerCore must be saved before creating relationship")
    ...
    @Transactional
    override fun createTeamPlayerRelationshipsBatch(
        teamCore: TeamCore,
        playerCores: List<PlayerCore>,
    ) {
        ...
        if (newRelationships.isNotEmpty()) {
            ...
            log.info(
                "Batch created ${newRelationships.size} team-player relationships: teamId=$teamId, playerIds=${newRelationships.map {
                    it.player
                        ?.id
                }}",
            )
    ...
    @Transactional
    override fun updateTeamPlayerRelationships(
        teamCore: TeamCore,
        playerCores: List<PlayerCore>,
        playerApiIds: List<Long>,
    ) {
        val teamId = teamCore.id ?: throw IllegalStateException("TeamCore must have an ID")
        ...
        if (newRelationships.isNotEmpty()) {
            teamPlayerCoreRepository.saveAll(newRelationships)
            log.info(
                "Added ${newRelationships.size} players to team: teamId=$teamId, playerIds=${newRelationships.map {
                    it.player
                        ?.id
                }}",
            )
        ...
        if (relationshipsToRemove.isNotEmpty()) {
            ...
            log.info(
                "Removed ${relationshipsToRemove.size} players from team: teamId=$teamId, playerIds=${relationshipsToRemove.map {
                    it.player
                        ?.id
                }}",
            )

...
class PlayerApiSportsWithCoreSyncerIntegrationTest {
    ...
    fun `대량의 선수 데이터를 배치로 처리할 때 성능이 안정적으로 유지되어야 한다`() {
        ...
        val playerDtos = (1..40).map { index ->
            createPlayerDto(
                apiId = (1000 + index).toLong(),
                name = "Player $index",
                position = if (index % 4 == 0) "Forward" else if (index % 4 == 1) "Midfielder" else if (index % 4 == 2) "Defender" else "Goalkeeper"
            )
        }
    ...
    fun `반복적인 업데이트 후에도 팀에 속한 선수 목록이 정확하게 유지되어야 한다`() {
        ...
        val removedPlayerRelations = teamPlayerCoreRepository.findByTeamIdAndPlayerId(testTeamCore.id!!, removedPlayer!!.playerCore!!.id!!)
    ...
    fun `기존 선수 업데이트와 새 선수 생성이 혼재되어도 정상 처리되어야 한다`() {
        ...
        // 1. 기존 선수가 있는 팀 생성
        val existingPlayer = PlayerApiSports(
            apiId = 101L,
            name = "Old Player",
            position = "Forward"
        )
    ...
    private fun createPlayerDto(apiId: Long, name: String?, position: String? = null): PlayerApiSportsCreateDto {
    ...
    private fun savePlayerCoreAndApiWithRelation(apiId: Long, name: String): PlayerApiSports {
        // PlayerCore 생성
        ...
        // PlayerApiSports 생성
        val playerApiSports = PlayerApiSports(
            playerCore = savedPlayerCore,
            apiId = apiId,
            name = name,
            position = "Forward"

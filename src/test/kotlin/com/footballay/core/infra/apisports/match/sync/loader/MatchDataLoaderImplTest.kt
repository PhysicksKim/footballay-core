...
class MatchDataLoaderImplTest {
    ...
    fun `홈팀 선수들이 올바르게 로드된다`() {
        ...
        log.info("로드된 모든 선수들: {}", entityBundle.allMatchPlayers.map { "${it.value.name} (팀: ${it.value.matchTeam?.id})" })
        
        // then
        val homeTeamPlayers = entityBundle.allMatchPlayers.filter { it.value.matchTeam?.id == matchEntities.homeMatchTeam.id }
    ...
    fun `원정팀 선수들이 올바르게 로드된다`() {
        ...
        // then
        val awayTeamPlayers = entityBundle.allMatchPlayers.filter { it.value.matchTeam?.id == matchEntities.awayMatchTeam.id }

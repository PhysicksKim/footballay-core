...
class TeamApiSportsSyncerIntegrationTest {
    ...
    fun `DTO 목록에 없는 팀은 리그와의 연관관계가 제거되어야 한다`() {
        ...
        // 명시적으로 연관관계 삭제 확인
        val removedTeamRelations = leagueTeamCoreRepository.findByLeagueIdAndTeamId(leagueCore.id!!, removedTeamCore!!.id!!)
    ...
    fun `반복적인 업데이트 후에도 리그에 속한 팀 목록이 정확하게 유지되어야 한다`() {
        ...
        val removedTeamRelations = leagueTeamCoreRepository.findByLeagueIdAndTeamId(leagueCore.id!!, removedTeam!!.id!!)
        assertEquals(0, removedTeamRelations.size)
    }

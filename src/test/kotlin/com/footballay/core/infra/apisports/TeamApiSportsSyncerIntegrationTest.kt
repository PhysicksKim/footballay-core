...
class TeamApiSportsSyncerIntegrationTest {
    ...
    fun `리그에 새로운 팀들을 추가하면 연관관계가 정상적으로 설정되어야 한다`() {
        ...
        val apiTeamDtos = listOf(
            createTeamDto(101L, MANUTD),
            createTeamDto(102L, LIVERPOOL)
        )
        log.info(apiTeamDtos.toString())

        // When
        teamApiSportsSyncer.saveTeamsOfLeague(leagueApiId, apiTeamDtos)
        flushAndClear()
    ...
    fun `DTO 목록에 없는 팀은 리그와의 연관관계가 제거되어야 한다`() {
        ...
        // Then
        // 101L 팀은 남아있고, 102L 팀은 연관관계만 제거되어야 함
        val leagueTeamRelations = leagueTeamCoreRepository.findByLeagueId(leagueCore.id!!)
        val teamsInLeague = leagueTeamRelations.mapNotNull { it.team }
        log.info("Teams in league after sync: ${teamsInLeague.map { it.name }}")
        val findAllRealtions = leagueTeamCoreRepository.findAll();
        log.info("All league-team relations: ${findAllRealtions.map { "${it.league.name} - ${it.team.name}" }}")

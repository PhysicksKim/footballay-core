...
class TeamApiSportsWithCoreSyncerIntegrationTest {
    ...
    fun `같은 Venue를 사용하는 여러 팀이 모두 올바르게 연결되어야 한다`() {
        ...
        val sharedVenue = VenueApiSportsCreateDto(
            apiId = 1001L,
            name = "Old Trafford",
            address = "Sir Matt Busby Way",
            city = "Manchester",
    ...
    fun `기존 Venue 업데이트와 새 Venue 생성이 혼재되어도 정상 처리되어야 한다`() {
        ...
        // 1. 기존 Venue가 있는 팀 생성
        val existingVenue = VenueApiSports(
            apiId = 3001L,
            name = "Old Stadium",
            address = "Old Address",
            city = "Old City",
        ...
        val existingTeam = TeamApiSports(
            apiId = 101L,
        ...
        val updatedVenueDto = VenueApiSportsCreateDto(
        ...
        val newVenueDto = VenueApiSportsCreateDto(
            apiId = 3002L,
            name = "New Stadium",
            address = "New Address",
            city = "New City",
    ...
    fun `preventUpdate가 true인 Venue는 업데이트되지 않아야 한다`() {
        ...
        val protectedVenue = VenueApiSports(
            apiId = 4001L,
        ...
        // 업데이트 시도할 DTO
        val updateVenueDto = VenueApiSportsCreateDto(
            apiId = 4001L,
    ...
    fun `대량의 팀 데이터를 배치로 처리할 때 성능이 안정적으로 유지되어야 한다`() {
        ...
        val largeTeamDtos = (1..100).map { index ->
            val venueDto: VenueApiSportsCreateDto? = if (index % 3 == 0) { // 3의 배수 팀만 Venue 있음

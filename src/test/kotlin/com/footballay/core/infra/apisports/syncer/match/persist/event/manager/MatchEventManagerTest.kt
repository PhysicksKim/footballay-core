package com.footballay.core.infra.apisports.syncer.match.persist.event.manager
...
class MatchEventManagerTest {
    ...
    fun `processMatchEvents_should_process_events_successfully`() {
        ...
        every { matchEventRepository.saveAll(any()) } returns savedEvents
    ...
    fun `processMatchEvents_should_create_empty_events_on_save_failure`() {
        ...
        every { matchEventRepository.saveAll(any()) } throws RuntimeException("Database error")
    ...
    fun `processMatchEvents_should_handle_deletions`() {
        ...
        every { matchEventRepository.saveAll(any()) } returns emptyList()

        // when
        val result = matchEventManager.processMatchEvents(eventDto, entityBundle)

        // then
        verify { matchEventRepository.deleteAll(any()) }
    ...
    @Test
    @DisplayName("예외 발생 시 상위로 전파합니다")
    fun `processMatchEvents_should_propagate_exceptions`() {
        // given
        val eventDto = MatchEventSyncDto(
            events = listOf(createMockEventDto(sequence = 0))
        )
        every { matchEventRepository.saveAll(any()) } throws RuntimeException("Critical error")

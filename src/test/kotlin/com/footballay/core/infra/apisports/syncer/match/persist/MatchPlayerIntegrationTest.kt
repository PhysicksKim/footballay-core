...
class MatchPlayerIntegrationTest {
    ...
    fun `전체 MatchPlayer 처리 파이프라인이 올바르게 작동한다`() {
        ...
        val changeSet = MatchPlayerChangePlanner.planChanges(collectedDtos, entityKeyMap, mockUidGenerator, null, null) // 3단계: 변경 계획
        ...
        assertThat(createdNames).containsExactlyInAnyOrder(
            "Home Player 2", "Away Player 1", "Away Player 2", 
            "Sub Player", "Referee", "Coach"
        )
    ...
    fun `단순한 시나리오에서 생성_업데이트_삭제가 모두 올바르게 처리된다`() {
        ...
        context.lineupMpDtoMap["mp_id_123"] = createMockPlayerDto(
            apiId = 123L, name = "Updated Player", number = 10, position = "Forward"
        )
        
        // 이벤트: 1명 (새로운 선수)
        context.eventMpDtoMap["mp_id_456"] = createMockPlayerDto(
            apiId = 456L, name = "New Player"
        )
        
        // 기존 엔티티: 1명 매칭 + 1명 고아
        val existingEntities = listOf(
            createMockMatchPlayerEntity(apiId = 123L, name = "Old Player", number = 9, position = "Defender"), // 업데이트
    ...
    fun `변경사항이 없는 경우 빈 변경 계획이 반환된다`() {
        ...
        context.lineupMpDtoMap["mp_id_123"] = createMockPlayerDto(
            apiId = 123L, name = "Same Player", number = 10, position = "Forward"
        )

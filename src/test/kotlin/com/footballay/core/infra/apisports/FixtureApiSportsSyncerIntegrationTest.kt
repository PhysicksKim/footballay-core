...
class FixtureApiSportsSyncerIntegrationTest {
    ...
    fun `기본적인 경기 저장 - 정상 플로우 검증`() {
        // given: 새로운 경기 데이터
        val fixtureDto = createBasicFixtureDto(
            fixtureApiId = 1001L,
    ...
    fun `Venue 포함 경기 저장 - Venue 처리 검증`() {
        ...
        val fixtureDto = createFixtureWithVenue(
            fixtureApiId = 1002L,
    ...
    private fun createAndSaveTeamApiSports(apiId: Long, name: String, teamCore: TeamCore) {
    ...
    private fun verifyVenueCreated(venueApiId: Long, expectedName: String) {

...
@Transactional
class LeagueApiSportsSyncerIntegrationTest {

    val log = logger()

    @Autowired
    private lateinit var leagueApiSportsSyncer: LeagueApiSportsSyncer
    ...
    fun `새로운 리그들을 추가하면 LeagueCore와 LeagueApiSports가 정상적으로 생성되어야 한다`() {
        ...
        log.info("Testing new leagues: ${leagueDtos.map { "${it.name}(${it.apiId})" }}")

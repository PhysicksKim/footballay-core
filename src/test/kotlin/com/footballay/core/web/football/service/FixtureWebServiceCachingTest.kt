...
import com.footballay.core.web.football.cache.FixtureWebCacheEntry
...
class FixtureWebServiceCachingTest {
    ...
    fun `getFixtureLiveStatus - cache hit 이고 etag 가 같으면 NotModified 를 반환한다`() {
        val cached =
            FixtureWebCacheEntry(
                ...
                etagHash = "etag-1",
                updatedAt = Instant.parse("2026-04-24T00:00:00Z"),
            )
        every { cacheManager.find("fixture-1", FixturePollingEndpoint.STATUS) } returns cached
    ...
    fun `getFixtureLiveStatus - cache miss 면 조회 후 저장하고 Ok 를 반환한다`() {
        ...
        every { cacheManager.find("fixture-1", FixturePollingEndpoint.STATUS) } returns null
    ...
    fun `getFixtureLiveStatus - bypass cache read 이면 캐시가 있어도 원본 조회를 강제한다`() {
        val cached =
            FixtureWebCacheEntry(
                snapshotJson = """{"fixtureUid":"fixture-1","liveStatus":{"shortStatus":"cached"}}""",
                etagHash = "etag-cached",
                updatedAt = Instant.parse("2026-04-24T00:00:00Z"),
            )
        val model =
            FixtureLiveStatusModel(
    ...
    fun `getFixtureLiveStatus - cache miss 이고 조회 실패면 Fail 을 반환한다`() {
        ...
        every { cacheManager.find("missing", FixturePollingEndpoint.STATUS) } returns null

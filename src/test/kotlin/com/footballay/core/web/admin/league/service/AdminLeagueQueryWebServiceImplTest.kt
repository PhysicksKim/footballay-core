...
class AdminLeagueQueryWebServiceImplTest {
    ...
    fun `available=true인 리그만 조회되어야 한다`() {
        // Given
        val availableLeague1 = createLeagueCore("test_league_1", "Premier League", true, MatchCollect.FINISHED)
        ...
        // LeagueApiSports 생성 (available 리그만)
        val apiSports1 = createLeagueApiSports(39L, availableLeague1, "Premier League", true)
        val apiSports2 = createLeagueApiSports(140L, availableLeague2, "La Liga", true)

        leagueApiSportsRepository.saveAll(listOf(apiSports1, apiSports2))
        em.flush()
        em.clear()
        ...
        assertThat(result.first { it.uid == "test_league_1" }.matchCollect).isEqualTo(MatchCollect.FINISHED)
        assertThat(result.first { it.uid == "test_league_2" }.matchCollect).isEqualTo(MatchCollect.NONE)
    ...
    private fun createLeagueCore(
        ...
        name: String,
        available: Boolean,
        matchCollect: MatchCollect = MatchCollect.NONE,
    ): LeagueCore =
        LeagueCore(
            uid = uid,
            name = name,

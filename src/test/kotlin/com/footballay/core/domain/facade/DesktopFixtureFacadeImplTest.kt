...
class DesktopFixtureFacadeImplTest {
    ...
    @Test
    fun `exact 모드 - mock data option이 있으면 mock fixture를 포함한다`() {
        val targetDate = Instant.parse("2025-03-05T10:00:00Z")
        createFixtureWithApiSports("api-option", targetDate)
        createFixtureWithMockBackbone("mock-option", targetDate.plus(3, ChronoUnit.HOURS))
        ...
        assertThat(defaultResult).isInstanceOf(DomainResult.Success::class.java)
        assertThat((defaultResult as DomainResult.Success).value.map { it.uid })
            .containsExactly("api-option")
    ...
    private fun createFixtureWithMockBackbone(
        uid: String,
        kickoff: Instant,
    ): FixtureCore {
        val fixtureCore =
        ...
        mockBackboneFixtureRepository.save(
            MockBackboneFixture(
                ...
                fixture = fixtureCore,
                initialStatusCode = fixtureCore.statusCode,
                initialKickoff = fixtureCore.kickoff,

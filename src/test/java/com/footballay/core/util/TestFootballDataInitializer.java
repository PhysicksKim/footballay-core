...
public class TestFootballDataInitializer {
    ...
    public List<String> getPlayerIds(Map<String, String> dataIdMap) {
        return dataIdMap.keySet().stream().filter(key -> key.contains("playerId")).map(dataIdMap::get).sorted().toList();
    }

    public List<String> getLeagueId(Map<String, String> dataIdMap) {
        return dataIdMap.keySet().stream().filter(key -> key.contains("leagueId")).map(dataIdMap::get).sorted().toList();
    }

    public List<String> getHomeTeamId(Map<String, String> dataIdMap) {
        return dataIdMap.keySet().stream().filter(key -> key.contains("homeTeamId")).map(dataIdMap::get).sorted().toList();
    }

    public List<String> getAwayTeamId(Map<String, String> dataIdMap) {
        return dataIdMap.keySet().stream().filter(key -> key.contains("awayTeamId")).map(dataIdMap::get).sorted().toList();
    }

    public List<String> getFixtureId(Map<String, String> dataIdMap) {
        return dataIdMap.keySet().stream().filter(key -> key.contains("fixtureId")).map(dataIdMap::get).sorted().toList();
    }
    ...
    public TestFootballDataInitializer(final LeagueRepository leagueRepository, final TeamRepository teamRepository, final LeagueTeamRepository leagueTeamRepository, final FixtureRepository fixtureRepository, final PlayerRepository playerRepository, final EntityManager em, final TeamPlayerRepository teamPlayerRepository, final LiveStatusRepository liveStatusRepository) {

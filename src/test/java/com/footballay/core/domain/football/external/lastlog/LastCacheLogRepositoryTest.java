...
class LastCacheLogRepositoryTest {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LastCacheLogRepositoryTest.class);
    @Autowired
    ...
    void success_API_CACHE() {
        ...
        LastCacheLog lastCacheLog = LastCacheLog.builder().apiCacheType(ApiCacheType.LEAGUE).parametersJson(Map.of("leagueId", (int) LeagueId.EPL)).lastCachedAt(cachedAt).build();
        // when
    ...
    void success_multipleApiCacheEntity_And_FindOne() {
        ...
        LastCacheLog leagueCache = LastCacheLog.builder().apiCacheType(ApiCacheType.LEAGUE).parametersJson(Map.of("leagueId", (int) LeagueId.EPL)).lastCachedAt(leagueCacheLDT).build();
        ZonedDateTime teamCacheLDT = ZonedDateTime.now();
        LastCacheLog teamCache = LastCacheLog.builder().apiCacheType(ApiCacheType.TEAM).parametersJson(Map.of("teamId", (int) TeamId.MANCITY)).lastCachedAt(teamCacheLDT).build();
        ...
        em.clear();
        Optional<LastCacheLog> teamFind = repository.findLastCacheLogByApiCacheTypeAndParametersJson(ApiCacheType.TEAM, Map.of("teamId", (int) TeamId.MANCITY));
        Optional<LastCacheLog> leagueFind = repository.findLastCacheLogByApiCacheTypeAndParametersJson(ApiCacheType.LEAGUE, Map.of("leagueId", (int) LeagueId.EPL));
        List<LastCacheLog> findAll = repository.findAll();
        // then
        log.info("Cache _StandingResponseData :: {}", leagueCache);
        log.info("Cache _Team :: {}", teamCache);
        log.info("Find _StandingResponseData :: {}", leagueFind);

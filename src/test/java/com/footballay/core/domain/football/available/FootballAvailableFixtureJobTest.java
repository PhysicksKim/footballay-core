...
public class FootballAvailableFixtureJobTest {
    ...
    public void testJobsRemovedWhenAvailableFixtureIsRemoved() throws Exception {
        ...
        waitUntilJobAllRemoved(lineupJobKey, liveMatchJobKey);
    ...
    public void testPostMatchJobRemovedWhenAvailableFixtureIsRemoved() throws Exception {
        ...
        waitUntilJobAllRemoved(lineupJobKey, liveMatchJobKey, postMatchJobKey);
    ...
    private void waitUntilJobAllRemoved(JobKey... jobKeys) {
        await().atMost(10, TimeUnit.SECONDS).untilAsserted(() -> {
            for (JobKey jobKey : jobKeys) {
                boolean exists = scheduler.checkExists(jobKey);
                assertThat(exists).isFalse(); // Assert that the job does not exist
            }
        });
    }

...
class PlayerCustomPhotoRepositoryTest {
    ...
    void setUp() {
        // User 생성
        User user1 = User.builder().username("user1").password("password1").enabled(true).build();
        User user2 = User.builder().username("user2").password("password2").enabled(true).build();
        userRepository.save(user1);
        userRepository.save(user2);
        // PreferenceKey 생성
        preferenceKey = PreferenceKey.builder().keyhash("prefKey1").user(user1).build();
        ...
        // Player 엔티티 생성
        player1 = Player.builder().id(100L).name("Player 1").build();
        player2 = Player.builder().id(101L).name("Player 2").build();
        player3 = Player.builder().id(102L).name("Player 3").build();
        em.persist(player1);
        em.persist(player2);
        em.persist(player3);
        // UserFilePath 생성
        userFilePath = UserFilePath.builder().user(user1).domain("https://example.com/").prefixPath("/prefixPath/").suffixPath("/suffixPath/").userPathHash("userHashKey123").userPathCategory(UserPathCategory.PLAYER_CUSTOM_PHOTO).build();
    ...
    private PlayerCustomPhoto createAndSavePlayerCustomPhoto(PreferenceKey preferenceKey, Player player) {
        PlayerCustomPhoto playerCustomPhoto = PlayerCustomPhoto.builder().preferenceKey(preferenceKey).player(player).userFilePath(userFilePath).fileName("player" + player.getId() + ".jpg").isActive(true).build();

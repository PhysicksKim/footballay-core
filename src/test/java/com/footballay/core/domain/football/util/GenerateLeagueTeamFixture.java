...
public class GenerateLeagueTeamFixture {
    ...
    public static class LeagueTeamFixture {
        ...
        public LeagueTeamFixture(final League league, final Team home, final Team away, final Fixture fixture) {
            this.league = league;
            this.home = home;
            this.away = away;
    ...
    private static LeagueTeamFixture generateFirstSet() {
        League league = League.builder().leagueId(leagueId).name(name).koreanName(koreanName).logo(logo).currentSeason(currentSeason).build();
        Team home = Team.builder().id(homeId).name(homeName).koreanName(homeKoreanName).logo(homeLogo).build();
        Team away = Team.builder().id(awayId).name(awayName).koreanName(awayKoreanName).logo(awayLogo).build();
        Fixture fixture = 
        // .status(status)
        Fixture.builder().fixtureId(fixtureId).referee(referee).timezone(timezone).date(date).timestamp(timestamp).homeTeam(home).awayTeam(away).league(league).build();
        return new LeagueTeamFixture(league, home, away, fixture);
    }

    private static LeagueTeamFixture generateSecondSet() {
        League league = League.builder().leagueId(leagueId).name(name).koreanName(koreanName).logo(logo).currentSeason(currentSeason).build();
        Team home = Team.builder().id(homeId2).name(homeName2).koreanName(homeKoreanName2).logo(homeLogo2).build();
        Team away = Team.builder().id(awayId2).name(awayName2).koreanName(awayKoreanName2).logo(awayLogo2).build();
        Fixture fixture = 
        // .status(status)
        Fixture.builder().fixtureId(fixtureId2).referee(referee).timezone(timezone).date(date).timestamp(timestamp).homeTeam(home).awayTeam(away).league(league).build();
        return new LeagueTeamFixture(league, home, away, fixture);
    }

    public static LeagueTeamFixture generateThirdSet() {
        League league = League.builder().leagueId(leagueId2).name(name2).koreanName(koreanName2).logo(logo2).currentSeason(currentSeason2).build();
        Team home = Team.builder().id(homeId3).name(homeName3).koreanName(homeKoreanName3).logo(homeLogo3).build();
        Team away = Team.builder().id(awayId3).name(awayName3).koreanName(awayKoreanName3).logo(awayLogo3).build();
        Fixture fixture = 
        // .status(status)
        Fixture.builder().fixtureId(fixtureId3).referee(referee).timezone(timezone).date(date).timestamp(timestamp).homeTeam(home).awayTeam(away).league(league).build();
        return new LeagueTeamFixture(league, home, away, fixture);
    }

...
class FootballApiCacheServiceTest {
    ...
    void success_leagueCaching() {
        ...
        // then
        League findLeague = leagueRepository.findById(eplId).orElseThrow(() -> new RuntimeException("테스트 에러! repository 에 저장된 league 가 없습니다. 캐싱에 실패했습니다."));
    ...
    void cacheTeamsOfLeague_UpdateIfAlreadyExist() {
        ...
        assertThat(belgium.getKoreanName()).isEqualTo("벨기에").withFailMessage("이미 저장되어 있던 팀은 api cache 에 의해 한글 이름 데이터가 변경되면 안됩니다");
    }
    ...
    void cacheTeamsOfLeague_AvoidDuplicateLeagueTeam() {
        ...
        // then
        long count = leagueTeams.stream().filter(leagueTeam -> leagueTeam.getTeam().getId() == belgiumId).count();
        assertThat(count).isEqualTo(1).withFailMessage("리그팀 연관관계가 중복 저장되었습니다.");
    }
    ...
    void cacheTeamSquad_AvoidDuplicateAndUpdate() {
        ...
        // then
        long count = teamPlayers.stream().filter(teamPlayer -> teamPlayer.getTeam().getId() == belgiumId && teamPlayer.getPlayer().getId() == PlayerId.De_Bruyne).count();
        assertThat(count).isEqualTo(1).withFailMessage("팀플레이어 연관관계가 중복 저장되었습니다.");
        Player deBruyne = playerRepository.findById(PlayerId.De_Bruyne).orElseThrow();
        assertThat(deBruyne.getKoreanName()).isEqualTo("케빈 데 브라위너").withFailMessage("한글 이름 필드가 덮어써져서는 안됩니다.");
    }

    private void saveEuroLeague() {
        final long leagueId = LeagueId.EURO;
        League league = League.builder().leagueId(leagueId).name("EURO").koreanName("유로").logo("https://static.domain.com/league/" + leagueId + ".png").available(true).currentSeason(2024).build();
    ...
    private void saveBelgium() {
        Team team = Team.builder().id(1L).name("Belgium").koreanName("벨기에").logo("https://media.api-sports.io/football/teams/1.png").build();
        teamRepository.save(team);
        League euro = leagueRepository.findById(LeagueId.EURO).orElseThrow();
        leagueTeamRepository.save(LeagueTeam.builder().league(euro).team(team).build());
        em.flush();
        em.clear();
    }

    private void saveKoreanInEuro() {
        Team korea = Team.builder().id(TeamId.SOUTH_KOREA).name("Korea").koreanName("대한민국").logo("https://media.api-sports.io/football/teams/2.png").build();
        teamRepository.save(korea);
        League euro = leagueRepository.findById(LeagueId.EURO).orElseThrow();
        leagueTeamRepository.save(LeagueTeam.builder().league(euro).team(korea).build());
        em.flush();
        em.clear();
    }
    ...
    private void saveBelgiumPlayer() {
        ...
        Player deBruyne = Player.builder().id(PlayerId.De_Bruyne).name("K. De Bruyne").koreanName("케빈 데 브라위너").photoUrl("https://media.api-sports.io/football/players/629.png").position("Midfielder").build();

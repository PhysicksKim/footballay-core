...
import static com.footballay.core.domain.football.external.fetch.response.FixtureSingleResponse._FixturePlayers.*;
...
class PlayerStatisticsServiceTest {
    ...
    void saveStats_UnregisteredPlayerNotInLineup_ShouldIgnoreStats() {
        ...
        boolean containsNoLineupUnregiPlayer = matchPlayers.stream().anyMatch(mp -> "No Lineup Unregistered Player Only In Stats".equals(mp.getUnregisteredPlayerName()));
        ...
        boolean invalidStatsCreated = matchPlayers.stream().filter(mp -> mp.getPlayerStatistics() != null).anyMatch(mp -> "No Lineup Unregistered Player Only In Stats".equals(mp.getUnregisteredPlayerName()));

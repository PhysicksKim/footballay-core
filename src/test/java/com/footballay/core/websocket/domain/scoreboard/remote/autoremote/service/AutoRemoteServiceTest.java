...
class AutoRemoteServiceTest {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(AutoRemoteServiceTest.class);
    @Autowired
    ...
    void Fail_UUIDNotCached() {
        ...
        // autoRemoteService.validateAndCacheUserToRedis(mockFirstPrincipal, userUUID.toString());
        // then
        Assertions.assertThrows(IllegalArgumentException.class, () -> autoRemoteService.connectToPrevFormedAutoRemoteGroup(mockFirstPrincipal, "nickname"), "잘못된 요청입니다. 사용자 UUID 또는 Principal 이 존재하지 않습니다.");
    }
    ...
    void Fail_AnonymousUserNotExist() {
        ...
        // then
        Assertions.assertThrows(IllegalArgumentException.class, () -> autoRemoteService.connectToPrevFormedAutoRemoteGroup(mockSecondPrincipal, "nickname"), "존재하지 않는 익명 유저입니다.");
    }
}

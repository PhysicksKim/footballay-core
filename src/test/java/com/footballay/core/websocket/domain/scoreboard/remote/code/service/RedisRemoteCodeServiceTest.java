...
public class RedisRemoteCodeServiceTest {
    ...
    void testAddAndRemoveSubscriber() {
        ...
        // then
        assertThat(addedSubscribers).containsEntry(hostPrincipalName, hostNickname).containsEntry(memberPrincipalName, memberNickname).size().isEqualTo(2);
        assertThat(removedSubscribers).containsEntry(hostPrincipalName, hostNickname).size().isEqualTo(1);
    }
    ...
    void fail_duplicate_nickname() {
        ...
        // then
        assertThrows(IllegalArgumentException.class, () -> redisRemoteCodeService.addSubscriber(remoteCode, mockSecondPrincipal.getName(), nickname1));
    }
